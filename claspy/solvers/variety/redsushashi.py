...
class Solver(Base):
  def _solve(self):
    possiblebridges = [0]+list(range(3,21))
    horiAns = utils.makeGrid(self.cols-1, self.rows, lambda: IntVar(possiblebridges))
    vertAns = utils.makeGrid(self.cols, self.rows-1, lambda: IntVar(possiblebridges))
