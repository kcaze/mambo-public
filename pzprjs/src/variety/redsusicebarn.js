//
// パズル固有スクリプト部 アイスバーン・アイスローム・アイスローム２版 redsusicebarn.js
//
(function(pidlist, classbase){
	if(typeof module==='object' && module.exports){module.exports = [pidlist, classbase];}
	else{ pzpr.classmgr.makeCustom(pidlist, classbase);}
}(
['redsusicebarn','icelom','icelom2'], {
//---------------------------------------------------------
// マウス入力系
"MouseEvent@redsusicebarn":{
		...
		if(this.puzzle.playmode){
			...
			if(this.mousestart || this.mousemove){
				if     (this.btn==='left') { this.inputarrow_line();}
					...
					if(this.pid==='redsusicebarn' || !cell.isNum()){
						this.inputIcebarn();
					}
					else{
						this.inputqnum();
			...
			else if(this.pid!=='redsusicebarn' && this.mouseend && this.notInputted()){
				this.inputqnum();
			}
		}
	},
...
"AreaIcebarnGraph:AreaGraphBase":{
	...
	setComponentRefs : function(obj, component){ obj.redsusicebarn = component;},
	getObjNodeList   : function(nodeobj){ return nodeobj.redsusicebarnnodes;},
	resetObjNodeList : function(nodeobj){ nodeobj.redsusicebarnnodes = [];},
...
Graphic:{
	...
	paint : function(){
		...
		if(this.pid!=='redsusicebarn'){ this.drawQuesNumbers();}

		this.drawBorderArrows();

		this.drawChassis();

		if(this.pid!=='redsusicebarn'){ this.drawTarget();}
...
"FileIO@redsusicebarn":{
	decodeData : function(){
		...
		this.decodeBorderLine_redsusicebarn();
	},
	encodeData : function(){
		var bd = this.board;
		...
		this.encodeBorderLine_redsusicebarn();
	},
...
FileIO:{
	decodeBorderLine_redsusicebarn : function(){
		this.decodeBorder( function(border,ca){
	...
	encodeBorderLine_redsusicebarn : function(){
		this.encodeBorder( function(border){
			var ca = "";
...
AnsCheck:{
	checklist : [
		...
		"checkDeadendRoad",
		"checkKeepInside",
		"checkFollowArrow@redsusicebarn",
		"checkNumberOrder@!redsusicebarn",
		...
		"checkAllArrow@redsusicebarn",
		"checkNoLineNumber@!redsusicebarn",
...
FailCode:{
	bkNoLine  : ["すべてのアイスバーンを通っていません。", "A redsusicebarn is not gone through."],
